        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <h4 class="header-title">Manage Data Kategori</h4>
                    <p class="sub-header">
                            ...
                            <div class="text-lg-end my-1 my-lg-0">
                                ...
                                <a href="{{ route('kategori.create') }}"
                                    class="btn btn-primary waves-effect waves-light mb-2"><i
                                        class="mdi mdi-plus me-1"></i>Add Data Kategori</a>
                            </div>
                        </div><!-- end col-->
                    </div>
                    <br>
                    ...
                    @if (isset($kategoris) && $kategoris->count() > 0)
                                ...
                                <tbody id="data-tbody">
                                    ...
                                    @foreach ($kategoris as $kategori)
                                        <tr class="data-row d-none">
                                            ...
                                            <td>
                                                <a href="{{ route('kategori.edit', $kategori->uuid) }}"
                                                    class="action-icon">
                                                    <i class="mdi mdi-square-edit-outline"></i>
                                                </a>
                                                <form action="{{ route('kategori.destroy', $kategori->uuid) }}"
                                                    method="POST" class="delete-role-form" style="display: inline-block;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="action-icon btn-delete-role"
                                                        data-role-name="{{ $kategori->nama }}" title="Hapus kategori"
                        ...
                        <div class="mt-3">
                            {!! $kategoris->appends(['search' => request('search'), 'per_page' => request('per_page')])->links('pagination::bootstrap-5') !!}
                    ...
                    @else
                        <div class="text-center p-4">
                            ...
                            <p class="text-muted">We couldn't find any kategori records. Try adding some new kategoris.</p>
                            <a href="{{ route('kategori.create') }}" class="btn btn-primary btn-sm mt-2">
                                <i class="mdi mdi-plus me-1"></i> Add New Kategori
                            </a>
                        </div>
                    @endif
            ...
            deleteButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    Swal.fire({
                        ...
                        html: `Nama Kategori "${roleName}"<br>will be permanently deleted.`,
                        icon: 'warning',
